...
class SpriteAtlas {
    ...
    addImage(name, pixels, width, height) {
        ...
        if (!(pixels instanceof Uint32Array)) {
            return console.error('Image provided in an invalid format. Supported formats are HTMLImageElement, ImageData, and ArrayBufferView.');
        }

        if (this.images[name]) {
            return console.error('An image with this name already exists.');
            // TODO throw error (or sliently overwrite?!)
        }

        const rect = this.allocateImage(width, height);
        if (!rect) {
            return console.error('There is not enough space to add this image.');
            // TODO throw error
        }
        ...
        // TODO handle the "wrap" arg more intelligently?
